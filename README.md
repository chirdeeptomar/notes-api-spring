...
database**. Pass the `dev` profile whenever you want data.

Caching and search are both on by default and need no configuration locally: `svc.cache.enabled`
and `svc.search.enabled` default to `true` (embedded Infinispan, local Lucene index) unless you
explicitly set either to `false` in `application.properties` or an env var — see
[Search § Backend](#backend) and [Caching § Embedded or remote](#embedded-or-remote) for switching
to a remote backend instead.

### Profile combinations

Spring profiles stack, so cache mode, search backend, and dev/prod are independent axes you
combine with a comma-separated `--spring.profiles.active` list:

```bash
# Local defaults: embedded Infinispan, local Lucene index, seeded H2 data
./gradlew bootRun --args='--spring.profiles.active=dev'

# Remote Infinispan cache, local Lucene search, dev data
./gradlew bootRun --args='--spring.profiles.active=dev,infinispan' ISPN_PASSWORD=password

# Embedded cache, remote OpenSearch search, dev data
./gradlew bootRun --args='--spring.profiles.active=dev,opensearch' SEARCH_HOSTS=localhost:9200

# Embedded cache, remote Elasticsearch search, dev data
./gradlew bootRun --args='--spring.profiles.active=dev,elasticsearch' SEARCH_HOSTS=localhost:9200

# Remote Infinispan cache AND remote OpenSearch search, dev data
./gradlew bootRun --args='--spring.profiles.active=dev,infinispan,opensearch' \
    ISPN_PASSWORD=password SEARCH_HOSTS=localhost:9200

# Prod database, remote Infinispan cache, remote Elasticsearch search
./gradlew bootRun --args='--spring.profiles.active=prod,infinispan,elasticsearch' \
    DB_PASSWORD=secret ISPN_PASSWORD=password SEARCH_HOSTS=localhost:9200
```

`opensearch` and `elasticsearch` are mutually exclusive (both set
`svc.search.mode`/`spring.jpa.properties.hibernate.search.backend.*` — whichever profile is listed
last on the active-profiles list wins). `infinispan` has no such conflict since caching only has
one remote option.
...
updating **two** places: `svc.tenant.*` and
...
server-side templating. To query a non-public tenant in GraphiQL, add `X-API-KEY` in its
**Headers** tab.
...
`svc.search.mode` selects which Hibernate Search backend indexes `Note.body`. **Lucene** is the
default: it needs no infrastructure to run. **OpenSearch** and **Elasticsearch** reach a remote
cluster instead — activate with the matching profile rather than by editing config:

```bash
SPRING_PROFILES_ACTIVE=dev,opensearch SEARCH_HOSTS=localhost:9200 ./gradlew bootRun
SPRING_PROFILES_ACTIVE=dev,elasticsearch SEARCH_HOSTS=localhost:9200 ./gradlew bootRun
```
...
OpenSearch's own version numbers diverged from Elasticsearch's after the 2021 fork. Override the
defaults with `SEARCH_HOSTS`, `SEARCH_PROTOCOL`, `SEARCH_USER`, `SEARCH_PASSWORD`,
`SEARCH_VERSION`, and, for a managed cluster reporting a version string outside Hibernate Search's
tested range, `SEARCH_VERSION_CHECK=false` to skip the startup compatibility check.

Tenant scoping (`hibernate.search.multi_tenancy.tenant_ids`, set in `application.properties`) is
honored identically in every mode, but a remote mode realizes it as one real cluster index per
tenant — shards and replicas — rather than a free local Lucene directory; worth planning for as
tenants are added.
...
Adding a tenant touches three places that must stay in sync — `svc.tenant.*`,
`hibernate.search.multi_tenancy.tenant_ids`, and the per-tenant caches (in `infinispan.xml` for
embedded mode, or the per-cache properties in `infinispan/hotrod-client.properties` for remote) — and requires
